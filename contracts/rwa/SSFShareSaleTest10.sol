...
interface ISSFShareTokenTest {
    function mint(address to, uint256 shares) external;
}

/**
 * @title SSFShareSaleTest10
 * @notice TEST VERSION - Allowlist-based share sale for mainnet E2E testing
 * @dev Uses real USDC but with minimal exposure parameters. NOT FOR PRODUCTION.
 * 
 * TEST PARAMETERS (test10):
 *   - Price: 10 USDC per share (minimal risk)
 *   - Max shares: 10 (total cap 100 USDC)
 *   - Per-wallet cap: 1 share (10 USDC max per wallet)
 *   - Allowlist capacity: 2 addresses (Chase wallet only)
 * 
 * USDC flows directly to treasury (contract never holds funds)
 */
contract SSFShareSaleTest10 is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // ============ VERSION ============
    string public constant VERSION = "SSFShareSale@1.0.1-test10";
    
    // ============ FROZEN TEST PARAMETERS ============
    uint256 public constant PRICE_PER_SHARE_USDC = 10_000_000; // 10 USDC (6 decimals)
    uint256 public constant MAX_SHARES = 10;
    uint256 public constant MAX_SHARES_PER_WALLET = 1;
    uint256 public constant MAX_ALLOWLIST = 2;
    
    // ============ IMMUTABLES ============
    IERC20 public immutable usdc;
    ISSFShareTokenTest public immutable shareToken;
    ...
    constructor(
        ...
        usdc = IERC20(_usdc);
        shareToken = ISSFShareTokenTest(_shareToken);
        treasury = _treasury;
        saleStart = _saleStart;
        saleEnd = _saleEnd;
        
        // Deploy paused by default - explicit unpause required
        _pause();
    ...
    function buy(uint256 shares) external nonReentrant whenNotPaused {
        ...
        // 4. Check per-wallet cap (1 share max for test)
     ...
     * @dev Enforces MAX_ALLOWLIST capacity (2 for test) when adding
